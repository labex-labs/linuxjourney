...
:::single-choice{#users-uid-purpose} Which identifier does the kernel primarily use to represent a user identity?
...
:::single-choice{#users-primary-supplementary-groups} How can one Linux account normally participate in groups?
...
:::single-choice{#users-process-access-identity} Which information is commonly considered when the kernel checks a process against file permissions?
...
:::single-choice{#users-root-uid} What numeric UID traditionally identifies the root account?
...
:::single-choice{#users-sudo-policy} What does `sudo` do before running a requested command?
