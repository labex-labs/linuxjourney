...
:::single-choice{#rsync-source-trailing-slash} What does the trailing slash in `rsync -a source/ destination/` signify?
...
:::single-choice{#rsync-archive-limit} Which metadata is not included in `-a` by itself?
...
:::single-choice{#rsync-dry-run-purpose} What does `--dry-run --itemize-changes` provide?
...
:::single-choice{#rsync-pull-direction} Which operand order pulls remote data into a local directory?
...
:::single-choice{#rsync-delete-effect} What can `--delete` do during synchronization?
