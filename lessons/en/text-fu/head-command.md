...
:::single-choice{#head-default-lines} What does `head events.log` print by default?
...
:::single-choice{#head-five-lines} Which command displays the first five lines of `report.txt`?
...
:::single-choice{#head-first-bytes} Which command writes the first 100 bytes of `payload.bin` to stdout?
...
:::single-choice{#head-pipeline-preview} In `generate-report | head -n 5`, what does `head` read?
...
:::single-choice{#head-suppress-filename-headers} Which option suppresses filename headers when `head` reads several files?
