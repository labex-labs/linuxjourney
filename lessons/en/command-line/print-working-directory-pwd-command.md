...
:::single-choice{#identify-root-subdirectories} In the directory tree above, how are `home` and `etc` related to `/`?
...
:::single-choice{#recognize-absolute-path} What makes `/home/pete/Movies` an absolute path?
...
:::single-choice{#expand-pwd-name} What does `pwd` stand for?
...
:::single-choice{#check-location-without-changing-it} Which action checks your current directory without changing it?
