...
:::single-choice{#stdin-descriptor-number} Welcher Dateideskriptor steht konventionsgemäß für die Standardeingabe?
...
:::single-choice{#stdin-from-file} Welcher Befehl lässt `sort` seine Standardeingabe aus `names.txt` lesen?
...
:::single-choice{#stdin-not-command-argument} Warum lässt `wc -l < peanuts.txt` den Namen `peanuts.txt` normalerweise in seiner Ausgabe weg?
...
:::single-choice{#stdin-and-stdout-files} Welche Datei liefert in `cat < input.txt > output.txt` die Standardeingabe und welche empfängt die Standardausgabe?
