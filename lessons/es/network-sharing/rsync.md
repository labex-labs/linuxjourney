...
:::single-choice{#rsync-source-trailing-slash} ¿Qué significa la barra final de `rsync -a source/ destination/`?
...
:::single-choice{#rsync-archive-limit} ¿Qué metadato no incluye `-a` por sí solo?
...
:::single-choice{#rsync-dry-run-purpose} ¿Qué proporciona `--dry-run --itemize-changes`?
...
:::single-choice{#rsync-pull-direction} ¿Qué orden de operandos obtiene datos remotos en un directorio local?
...
:::single-choice{#rsync-delete-effect} ¿Qué puede hacer `--delete` durante la sincronización?
