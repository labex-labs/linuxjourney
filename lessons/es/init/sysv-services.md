...
¿Qué suele ejecutar el comando `service`?
...
¿Qué comando solicita que se detenga el servicio SysV `peanut`?
...
¿Por qué no se debe suponer que `reload` equivale a `restart`?
...
¿Ejecutar `service SERVICE start` habilita necesariamente el servicio para futuros arranques?
