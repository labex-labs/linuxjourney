...
:::single-choice{#dev-directory-device-node-purpose} ¿Qué ocurre cuando un programa abre un nodo de dispositivo bajo `/dev`?
...
:::single-choice{#dev-directory-null-behavior} ¿Qué hace `/dev/null` con los datos que se escriben en él?
...
:::single-choice{#dev-directory-persistent-link} ¿Por qué puede preferir un administrador `/dev/disk/by-id/...` a `/dev/sda` en una configuración?
...
:::single-choice{#dev-directory-direct-write-risk} ¿Por qué debes evitar escribir datos arbitrarios en un nodo de dispositivo desconocido?
