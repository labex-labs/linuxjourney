...
:::single-choice{#device-names-sdb-first-partition} Según el patrón de nombres `sd`, ¿qué ruta designa la partición 1 de `/dev/sdb`?
...
:::single-choice{#device-names-nvme-partition} ¿Qué ruta designa la partición 2 de `/dev/nvme0n1`?
...
:::single-choice{#device-names-device-mapper-link} ¿Qué ubicación suele proporcionar enlaces descriptivos para los dispositivos del mapeador de dispositivos?
...
:::single-choice{#device-names-persistent-config} ¿Por qué los enlaces de `/dev/disk/by-id/` suelen ser preferibles a `/dev/sdX` en una configuración específica de un dispositivo?
...
:::single-choice{#device-names-zero-read} ¿Qué produce la lectura de `/dev/zero`?
