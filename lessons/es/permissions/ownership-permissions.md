...
¿Qué orden cambia el usuario propietario de `myfile` a `patty` y deja su grupo sin cambios?
...
¿Qué cambia `chgrp whales myfile`?
...
¿Qué especificación de propiedad asigna el usuario `patty` y el grupo `whales` en una orden `chown`?
...
Después de cambiar el grupo propietario de un archivo, ¿qué ocurre con sus bits normales de permisos de grupo?
