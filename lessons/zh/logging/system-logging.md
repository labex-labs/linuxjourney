...
:::single-choice{#system-logging-distinct-roles} 哪个部分决定已接收的消息存储到哪里或转发到哪里？
...
:::single-choice{#system-logging-file-absence} 缺少 `/var/log/syslog` 文件必然意味着什么？
...
:::single-choice{#system-logging-current-boot} 哪个选项将 `journalctl` 查询限制在当前启动？
...
:::single-choice{#system-logging-export-safety} 向外部分享日志摘录前应该做什么？
