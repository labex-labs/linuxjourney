...
:::single-choice{#define-ll-alias} 哪个 Bash 命令会在当前 shell 中把 `ll` 定义为 `ls -la` 的别名？
...
:::single-choice{#persist-bash-alias} 个人别名通常应定义在哪里，才能被未来的交互式非登录 Bash 会话加载？
...
:::single-choice{#inspect-command-alias} 哪个命令会显示 Bash 当前把 `ll` 解析为别名、函数、内建命令还是可执行文件？
...
:::single-choice{#bypass-ls-alias} 当前 Bash 会话有一个名为 `ls` 的别名。哪个命令会在一次调用中绕过它？
...
:::single-choice{#remove-current-alias} 哪个命令会从当前 Bash 会话中删除别名 `ll`？
