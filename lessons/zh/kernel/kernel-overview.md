...
:::single-choice{#kernel-overview-hardware-manager} 在 Linux 上，通常由哪一层协调设备驱动程序和硬件中断？
...
:::single-choice{#kernel-overview-scheduler-role} 内核调度器管理什么？
...
:::single-choice{#kernel-overview-root-user-mode} 普通的 root 用户应用程序会在内核模式下执行全部指令吗？
...
:::single-choice{#kernel-overview-system-call-boundary} 什么是系统调用？
