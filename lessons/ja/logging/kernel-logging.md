...
:::single-choice{#kernel-log-ring-buffer-limit} 古いカーネルイベントが現在の `dmesg` 出力にない場合があるのはなぜですか？
...
:::single-choice{#kernel-log-timestamp-caution} 変換済み `dmesg` の wall-clock timestamp を慎重に扱うべきなのはなぜですか？
...
:::single-choice{#kernel-log-previous-boot} 保存されている一つ前の boot のカーネルメッセージを要求するコマンドはどれですか？
...
:::single-choice{#kernel-log-warning-response} 一つのカーネル warning 行に対する最善の対応はどれですか？
