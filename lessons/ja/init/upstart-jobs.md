...
Upstart の状態出力で、`stop/waiting` は通常何を意味しますか？
...
ジョブ `peanuts` の手動開始を要求するコマンドはどれですか？
...
実行中の Upstart ジョブ `peanuts` の再起動を要求するコマンドはどれですか？
...
`initctl emit EVENT_NAME` を実行すると、何が起こり得ますか？
