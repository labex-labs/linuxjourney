...
通常の `/etc/hosts` の対応付け行では、最初に何を書きますか？
...
システム resolver が DNS より先に `/etc/hosts` を参照するかどうかは、何で決まりますか？
...
通常のシステム名前解決が hosts ファイルのエントリを認識するか調べるのに適したツールはどれですか？
...
公開サービス名を上書きせず、専用のテスト名を使うのはなぜですか？
...
`/etc/resolv.conf` を編集する前に何をすべきですか？
