...
接続済みの `scope link` 宛先には、通常どのように到達しますか？
...
`192.168.224.50` に、通常より具体的に一致するルートはどれですか？
...
`ip route show` だけではアプリケーションの経路を説明できない場合があるのはなぜですか？
