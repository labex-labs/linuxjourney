...
스크립트가 `eth0`을 가정하지 말고 찾아야 하는 이유는 무엇입니까?
...
`ip address add`만으로 재부팅 후 영구 적용이 보장됩니까?
...
`ip address`에 새 주소가 보이는 것보다 더 나은 증거는 무엇입니까?
