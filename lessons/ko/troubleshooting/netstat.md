...
:::single-choice{#netstat-ss-numeric} 소켓 문제 해결 중 `-n`을 사용하는 이유는 무엇입니까?
...
:::single-choice{#netstat-services-file-limit} `https 443/tcp` 같은 `/etc/services` 항목은 무엇을 확립합니까?
...
:::single-choice{#netstat-close-wait-owner} `CLOSE-WAIT`에서 어느 쪽이 아직 소켓을 닫아야 합니까?
...
:::single-choice{#netstat-queue-snapshot} 큰 소켓 큐 스냅샷 하나만으로 진단하기에 부족한 이유는 무엇입니까?
...
:::single-choice{#netstat-listener-limit} 포트 443의 TCP 리스너가 입증하지 못하는 것은 무엇입니까?
