...
:::single-choice{#vim-insert-return-normal} 일반적으로 입력 모드에서 일반 모드로 돌아가는 키는 무엇인가요?
...
:::single-choice{#vim-insert-before-cursor} 커서 바로 앞에서 입력 모드로 들어가는 일반 모드 키는 무엇인가요?
...
:::single-choice{#vim-insert-first-nonblank} 현재 줄의 첫 번째 비공백 문자 앞에서 삽입을 시작하는 일반 모드 명령은 무엇인가요?
...
:::single-choice{#vim-append-line-end} 현재 줄 끝으로 이동하고 입력 모드로 들어가는 일반 모드 명령은 무엇인가요?
...
:::single-choice{#vim-open-line-above} 현재 줄 위에 새 줄을 열고 입력 모드로 들어가는 일반 모드 명령은 무엇인가요?
