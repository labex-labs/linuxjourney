...
:::single-choice{#killing-processes-default-signal} `kill PID`가 기본적으로 요청하는 시그널은 무엇인가요?
...
:::single-choice{#killing-processes-pid-reuse} 시그널을 보내기 직전에 PID를 검사해야 하는 이유는 무엇인가요?
...
:::single-choice{#killing-processes-signal-zero} 성공한 `kill -0 PID`는 그 순간 무엇을 확인하나요?
...
:::single-choice{#killing-processes-kill-tradeoff} `SIGKILL`의 주된 운영상 단점은 무엇인가요?
