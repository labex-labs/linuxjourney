...
:::single-choice{#dns-tools-system-resolver} 설정된 시스템 이름 서비스 경로를 사용하는 명령은 무엇입니까?
...
:::single-choice{#dns-tools-record-type} IPv6 주소 레코드를 요청하는 쿼리는 무엇입니까?
...
:::single-choice{#dns-tools-noerror-empty} `NOERROR`에 빈 응답 섹션이 있을 수 있습니까?
...
:::single-choice{#dns-tools-aa-flag} `aa` 응답 플래그는 무엇을 뜻합니까?
...
:::single-choice{#dns-tools-tcp-test} `dig +tcp`는 무엇을 바꿉니까?
