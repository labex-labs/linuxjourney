...
:::single-choice{#auth-logs-file-location} 리눅스 인증 이벤트는 항상 어디에 저장되어야 합니까?
...
:::single-choice{#auth-logs-uid-inference} 이 레코드에서 `uid=1000`이 확립하는 사실은 무엇입니까?
...
:::single-choice{#auth-logs-failed-attempts} 반복되는 로그인 실패는 어떤 정보와 연관 지어야 합니까?
...
:::single-choice{#auth-logs-preservation} 조사 중 인증 증거는 어떻게 다뤄야 합니까?
