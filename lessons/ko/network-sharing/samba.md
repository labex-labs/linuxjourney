...
사용자가 Samba 공유를 통해 쓰려면 무엇이 허용해야 합니까?
...
`valid users = @teamshare`는 무엇을 나타냅니까?
...
독립형 서버에서 `smbpasswd -a alice`는 일반적으로 무엇을 합니까?
...
Samba 변경을 적용하기 전에 `testparm -s`를 실행하는 이유는 무엇입니까?
...
마운트 명령에 `password=...`를 직접 쓰면 안 되는 이유는 무엇입니까?
