...
직접 `insmod`를 사용하는 것보다 일반적으로 `modprobe`가 선호되는 이유는 무엇입니까?
...
`/etc/modprobe.d/`의 `options` 줄은 무엇을 합니까?
...
기본 modprobe `blacklist` 줄이 주로 억제하는 것은 무엇입니까?
...
이름으로 모듈의 의존성을 고려한 제거를 요청하는 명령은 무엇입니까?
