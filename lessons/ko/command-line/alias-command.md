...
:::single-choice{#define-ll-alias} 현재 쉘에서 `ll`을 `ls -la`의 별칭으로 정의하는 Bash 명령어는 무엇인가요?
...
:::single-choice{#persist-bash-alias} 이후 대화형 비로그인 Bash 세션이 개인 별칭을 불러오도록 보통 어디에 정의해야 하나요?
...
:::single-choice{#inspect-command-alias} Bash가 현재 `ll`을 별칭, 함수, 내장 명령어 또는 실행 파일 중 무엇으로 해석하는지 보여 주는 명령어는 무엇인가요?
...
:::single-choice{#bypass-ls-alias} 현재 Bash 세션에 `ls`라는 별칭이 있습니다. 한 번의 실행에서 이 별칭을 우회하는 명령어는 무엇인가요?
...
:::single-choice{#remove-current-alias} 현재 Bash 세션에서 `ll` 별칭을 제거하는 명령어는 무엇인가요?
