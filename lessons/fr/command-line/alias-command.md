...
Quelle commande Bash définit `ll` comme alias de `ls -la` dans le shell actuel ?
...
Où faut-il normalement définir un alias personnel pour que les futures sessions Bash interactives sans connexion le chargent ?
...
Quelle commande indique si Bash résout actuellement `ll` comme alias, fonction, commande intégrée ou exécutable ?
...
La session Bash actuelle possède un alias `ls`. Quelle commande le contourne pour une seule invocation ?
...
Quelle commande supprime l'alias `ll` de la session Bash actuelle ?
