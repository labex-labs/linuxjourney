...
Combien de champs une entrée normale de `/etc/fstab` contient-elle ?
...
Qu'identifie normalement `UUID=...` dans le champ source ?
...
Que demande la valeur `0` dans le sixième champ ?
...
Pourquoi faut-il confirmer l'accès au mode de secours avant de modifier une entrée fstab essentielle ?
...
Pourquoi `mount -a` ne constitue-t-il pas à lui seul une validation complète de fstab ?
