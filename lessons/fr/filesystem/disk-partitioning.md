...
Quelle affirmation à propos de la version actuelle de `fdisk` sous Linux est exacte ?
...
Pourquoi un nom de périphérique comme `/dev/sdb` ne suffit-il pas comme unique vérification de la cible ?
...
Qu'aide à afficher `print free` dans `parted` ?
...
Que crée la commande `mkpart` de `parted` ?
...
Lorsqu'un système de fichiers peut être réduit, quel ordre évite de tronquer ses données actives ?
