...
Pourquoi l'erreur de `ls /missing > results.txt` reste-t-elle normalement au terminal ?
...
Quelle commande remplace `errors.log` par les diagnostics de `find /restricted` tout en laissant stdout inchangé ?
...
Quelle redirection Bash envoie stdout et stderr de `command` vers `all.log` ?
...
Quelle commande Bash ajoute stdout et stderr de `build` à `build.log` ?
...
Que change `check-data 2> /dev/null` ?
