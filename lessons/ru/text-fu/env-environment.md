...
Если вы хотите, чтобы переменная окружения была доступна в каждой сессии терминала (даже после закрытия и повторного открытия терминала), вам необходимо добавить ее в файл запуска вашей оболочки. В случае с Bash (оболочка по умолчанию для многих дистрибутивов Linux), этот файл обычно называется `.bashrc` в вашем домашнем каталоге.
