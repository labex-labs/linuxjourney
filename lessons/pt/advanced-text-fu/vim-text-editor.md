...
:::single-choice{#vim-name-origin} O que significa o nome Vim?
...
:::single-choice{#vim-check-command} Qual comando verifica se o Bash consegue resolver o nome `vim` no momento?
...
:::single-choice{#vim-open-missing-path} O que normalmente acontece quando `vim draft.txt` indica um caminho que ainda não existe?
...
:::single-choice{#vim-guided-tutorial} Qual comando do shell inicia o tutorial guiado do Vim quando ele está instalado?
...
:::single-choice{#vim-abandon-practice-changes} Em um arquivo descartável de prática, qual comando do Vim fecha a janela atual e descarta suas alterações não salvas?
