...
:::single-choice{#kernel-log-ring-buffer-limit} Por que um evento antigo pode não aparecer no `dmesg` atual?
...
:::single-choice{#kernel-log-timestamp-caution} Por que horários convertidos do `dmesg` exigem cuidado?
...
:::single-choice{#kernel-log-previous-boot} Qual comando solicita mensagens do kernel no boot anterior retido?
...
:::single-choice{#kernel-log-warning-response} Qual é a melhor resposta a uma única linha de aviso do kernel?
